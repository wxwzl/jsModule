					   ...
					   addSubManager:function(name,eventManager){
						...
						addListener:function(types,selector,method,times){
						   ...
						   trigger:function(event){
								...
								var type=event.type,nodeName=target.nodeName.toLowerCase(),
								nodeType=target.nodeType;
								if ( nodeType === 3 ||nodeType === 8 || 
										(type=="click" && (nodeName== "input" || nodeName== "select") ) ) {
									return;
								}
							    var i,j,flag,catched=false,param;this.Behind=false;
							    var paramArray=[];
					...
					//true,将阻止其父事件管理器中的排在该管理器之后的子管理器的事件触发。false,反之
					  ...
					  * */
					  on:function(type,handler,times){
						  ...
						  var size=types.length,e=null;
						  for(var i=0;i<size;i++){
						  	e=this.trim(types[i]);
						  	if(this.handlers[e]==undefined){
								  this.handlers[e]=[];
							  }
							  this.handlers[e].push({"method":handler,"param":param,"time":times});
						  }
						  return this;
					  },
					  /**
					  ...
					  * */
					  off:function(type,handler){
						  if(handler!=undefined){
							  var handlers=this.handlers[type],i;
					  ...
					  * */
					  fire:function(type){
						  var handlers=this.handlers[type],i,handler=null;
						  var param=[];
						  if(arguments.length>1){
